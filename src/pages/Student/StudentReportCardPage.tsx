...
export function StudentReportCardPage() {
  ...
  const myAttend    = attendance.filter((a) => a.studentId === user?.id);
  ...
  if (!profile) {
    return (
      <div className="alert alert-warning mt-4">
        No student record found. Please contact the school office.
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="page-heading no-print">
        <div>
          ...
          <p>{profile.firstName} {profile.lastName} — {profile.className}</p>
        </div>
      ...
      <div className="report-card" ref={printRef} id="report-card-print">
        ...
        <div className="report-term-bar">
          ...
          <div><span className="report-term-key">Date Issued:</span> <strong>2026-06-27</strong></div>
          <div><span className="report-term-key">Class:</span> <strong>{profile.className}</strong></div>
        </div>
        ...
        <div className="report-profile-row">
          <img src={profile.profilePhoto} alt={profile.firstName} className="report-student-photo" />
          <div className="report-student-details">
            <h3 className="report-student-name">{profile.firstName} {profile.lastName}</h3>
            <table className="report-info-table">
              <tbody>
                <tr>
                  ...
                  <td>Gender</td>
                  <td><strong>{profile.gender}</strong></td>
                </tr>
                <tr>
                  <td><FiCalendar size={12} className="me-1" />Date of Birth</td>
                  <td><strong>{profile.dateOfBirth}</strong></td>
                  <td>Parent / Guardian</td>
                  <td><strong>{profile.parentName}</strong></td>
                </tr>
                <tr>
                  <td>Email</td>
                  <td><strong>{profile.email}</strong></td>
                  <td>Parent Phone</td>
                  <td><strong>{profile.parentPhone}</strong></td>
                </tr>
              </tbody>
            </table>
          </div>
